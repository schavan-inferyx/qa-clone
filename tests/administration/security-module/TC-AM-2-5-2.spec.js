...
let GROUP_UUID;
let CLONED_GROUP_UUID;

test.describe.serial("Administration - Group CRUD", () => {
  ...
  test.beforeEach(async ({ authentication }) => {
    ...
    await admin.navigateToGroup();

  });

  /* ================= CREATE ================= */

  test("Create Group", async ({ authentication }) => {
    ...
    await admin.fillGroupCreateForm({
      name: groupName,
    ...
    expect(toast).toBe("Group saved successfully");

    await admin.checkBreadcrumbContainsView("View");

    GROUP_UUID = await admin.getUUIDFromView();

    expect(GROUP_UUID).toBeTruthy();
  ...
  test("View", async ({ authentication }) => {
    ...
    await common.searchByUUID(GROUP_UUID);
  ...
  test("Clone", async ({ authentication }) => {
    ...
    CLONED_GROUP_UUID = uuid;

    expect(CLONED_GROUP_UUID).toBeTruthy();
  ...
  test("Delete", async ({ authentication }) => {
    ...
    await common.searchByUUID(CLONED_GROUP_UUID);
