...
let QUICKLINK_UUID;
let CLONED_QUICKLINK_UUID;

test.describe.serial("Administration - Quick Link CRUD", () => {
  ...
  test.beforeEach(async ({ authentication }) => {
    ...
    await admin.navigateToQuickLink();
  ...
  test("Create Quick Link", async ({ authentication }) => {

    const admin = new Admin(authentication);

    const quickLinkName = `QuickLink_${Date.now()}`;

    await admin.openAddForm();

    // await admin.checkBreadcrumbContainsAdd("Add");

    await admin.fillQuickLinkCreateForm({
      name: quickLinkName,
      url: "tester",
      account: "Inferyx Account",
      selectProducts: "Data Catalog",
    });
    ...
    QUICKLINK_UUID = await admin.getUUIDFromView();

    expect(QUICKLINK_UUID).toBeTruthy();
  ...
  test("View", async ({ authentication }) => {
    ...
    await common.searchByUUID(QUICKLINK_UUID);
  ...
  test("Clone", async ({ authentication }) => {
    ...
    CLONED_QUICKLINK_UUID = uuid;

    expect(CLONED_QUICKLINK_UUID).toBeTruthy();
  ...
  test("Delete", async ({ authentication }) => {
    ...
    await common.searchByUUID(CLONED_QUICKLINK_UUID);
