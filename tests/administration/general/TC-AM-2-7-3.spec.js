...
let CATEGORY_UUID;
let CLONED_CATEGORY_UUID;

test.describe.serial("Administration - Category CRUD", () => {
  ...
  test.beforeEach(async ({ authentication }) => {
    ...
    await admin.navigateToCategory();
  ...
  test("Create Category", async ({ authentication }) => {
    ...
    await admin.fillCategoryCreateForm({
      name: categoryName
    });
    ...
    expect(toast).toBe("Category saved successfully");

    await admin.checkBreadcrumbContainsView("View");

    CATEGORY_UUID = await admin.getUUIDFromView();

    expect(CATEGORY_UUID).toBeTruthy();
  ...
  test("View", async ({ authentication }) => {
    ...
    await common.searchByUUID(CATEGORY_UUID);
  ...
  test("Clone", async ({ authentication }) => {
    ...
    CLONED_CATEGORY_UUID = uuid;

    expect(CLONED_CATEGORY_UUID).toBeTruthy();
  ...
  test("Delete", async ({ authentication }) => {
    ...
    await common.searchByUUID(CLONED_CATEGORY_UUID);
