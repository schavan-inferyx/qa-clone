...
let USERGROUP_UUID;
let CLONED_USERGROUP_UUID;

test.describe.serial("Administration - User Group CRUD", () => {
  ...
  test.beforeEach(async ({ authentication }) => {
    ...
    await admin.navigateToUserGroup();
  ...
  test("Create User Group", async ({ authentication }) => {

    const admin = new Admin(authentication);

    const userGroupName = `UserGroup_${Date.now()}`;
    ...
    await admin.fillUserGroupCreateForm({
    ...
    expect(toast).toBe("UserGroup saved successfully");


    await admin.checkBreadcrumbContainsView("View");


    USERGROUP_UUID = await admin.getUUIDFromView();

    expect(USERGROUP_UUID).toBeTruthy();
  ...
  test("View", async ({ authentication }) => {
    ...
    await common.searchByUUID(USERGROUP_UUID);
  ...
  test("Clone", async ({ authentication }) => {
    ...
    CLONED_USERGROUP_UUID = uuid;

    expect(CLONED_USERGROUP_UUID).toBeTruthy();
  ...
  test("Delete", async ({ authentication }) => {
    ...
    await common.searchByUUID(CLONED_USERGROUP_UUID);
