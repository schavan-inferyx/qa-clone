...
let EXPORT_UUID;
let CLONED_EXPORT_UUID;

test.describe.serial("Administration - Migration Assist Export CRUD", () => {
  ...
  test.beforeEach(async ({ authentication }) => {
    ...
    await admin.navigateToMigrationAssist();
    await admin.navigateToMigrationExport();
  ...
  test("Create Migration Export", async ({ authentication }) => {

    const admin = new Admin(authentication);

    const exportName = `Export_${Date.now()}`;

    await admin.openAddForm();

    await admin.checkBreadcrumbContainsAdd("Add");

    await admin.fillMigrationExportCreateForm({
    ...
    EXPORT_UUID = await admin.getUUIDFromView();

    expect(EXPORT_UUID).toBeTruthy();
  ...
  test("View", async ({ authentication }) => {
    ...
    await common.searchByUUID(EXPORT_UUID);
  ...
  test("Clone", async ({ authentication }) => {
    ...
    CLONED_EXPORT_UUID = uuid;

    expect(CLONED_EXPORT_UUID).toBeTruthy();
  ...
  test("Delete", async ({ authentication }) => {
    ...
    await common.searchByUUID(CLONED_EXPORT_UUID);
